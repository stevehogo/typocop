...
export function renderNavigationIndex(data: GraphData): string {
  const lines = [
    ...
    `external_dependency_count: ${data.externalDependencies.length}`,
    "---",
    "",
    "# Code Graph Navigator",
    "",
    `**Symbols**: ${data.symbols.length}  |  **Clusters**: ${data.clusters.length}  |  **Processes**: ${data.processes.length}  |  **External Dependencies**: ${data.externalDependencies.length}`,
    "",
    "## Navigation",
    "",
    ...
    "- **[[04-external-dependencies|External Dependencies]]** — Indexed third-party packages and their dependents",
    "",
