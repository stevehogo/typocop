...
- [x] 3. Update `package.json` — bin, files, engines, scripts, and dependencies
...
- [x] 5. Write unit tests for `src/cli/main.ts`
  _Skills: `testing-patterns`, `tdd-workflow`, `typescript-expert`
  - [x] 5.1 Create `src/cli/main.test.ts` with `vi.mock` for `./index.js`
    - Mock `parseArgs`, `executeCLI`, `CLIValidationError`
    - Spy on `process.stderr.write` and `process.exit`
    - _Requirements: 1.3, 1.4, 1.5, 1.6_
  - [x] 5.2 Test success path: `executeCLI` resolves → `process.exit(0)`
    - _Requirements: 1.6_
  - [x] 5.3 Test `CLIValidationError` path: stderr receives message + `process.exit(1)`
    - _Requirements: 1.4_
  - [x] 5.4 Test unexpected error path: stderr receives message + `process.exit(1)`
    - _Requirements: 1.5_
  - [x] 5.5 Test `-e` with existing file: mock `existsSync` → `true`, mock `dotenv.config`; assert `dotenv.config` called before `parseArgs`
    - _Requirements: 9.3_
  - [x] 5.6 Test `-e` with missing file: mock `existsSync` → `false`; assert stderr write + `process.exit(1)` and `parseArgs`/`executeCLI` NOT called
    - _Requirements: 9.5_

- [x] 6. Write unit tests for `src/mcp/main.ts`
  _Skills: `testing-patterns`, `tdd-workflow`, `typescript-expert`
  - [x] 6.1 Create `src/mcp/main.test.ts` with `vi.mock` for `./index.js`
    - Mock `startMCPServer`
    - Spy on `process.stderr.write` and `process.exit`
    - _Requirements: 5.2, 5.3, 5.4_
  - [x] 6.2 Test success path: `startMCPServer` resolves → process does NOT call `process.exit`
    - _Requirements: 5.4_
  - [x] 6.3 Test failure path: `startMCPServer` rejects → stderr receives message + `process.exit(1)`
    - _Requirements: 5.3_
  - [x] 6.4 Test `-e` with existing file: mock `existsSync` → `true`, mock `dotenv.config`; assert `dotenv.config` called before `startMCPServer`
    - _Requirements: 9.4_
  - [x] 6.5 Test `-e` with missing file: mock `existsSync` → `false`; assert stderr write + `process.exit(1)` and `startMCPServer` NOT called
    - _Requirements: 9.6_

- [x] 7. Write property-based tests (fast-check)
  _Skills: `testing-patterns`, `typescript-expert`
  - [x] 7.1 Property 1 — CLI entry point propagates any error to stderr
  ...
  - [x] 7.2 Property 2 — MCP entry point propagates any error to stderr
    - Use `fc.string()` to generate arbitrary error messages
  ...
  - [x] 7.3 Property 3 — Unknown commands always exit with code 1
  ...
  - [x] 7.4 Property 4 — Env_Flag file-not-found always exits with code 1 (CLI)
  ...
  - [x] 7.5 Property 4 — Env_Flag file-not-found always exits with code 1 (MCP)
...
- [x] 9. Write smoke tests — `package.json` field assertions and shebang verification
  _Skills: `testing-patterns`
  - [x] 9.1 Create `tests/smoke/packaging.test.ts`
    ...
    - _Requirements: 1.1, 1.7, 2.1, 2.2, 2.5, 3.5, 3.6, 4.1, 8.3, 9.7_
  - [x] 9.2 Assert `.env.example` exists and contains all 9 variable names
    - Read `.env.example` with `node:fs` and assert each variable name is present
    - _Requirements: 6.5_

- [x] 10. Write integration tests — build output, permissions, and pack dry-run
  _Skills: `testing-patterns`, `nodejs-best-practices`
  - [x] 10.1 Create `tests/integration/packaging.test.ts`
    - After `pnpm run build`, assert `dist/cli/main.js` and `dist/mcp/main.js` exist
    - _Requirements: 2.3, 2.4_
  - [x] 10.2 Assert executable permission bits on both entry point files (POSIX only, skip on Windows)
    - Use `node:fs.statSync` and check `mode & 0o111`
    - _Requirements: 8.1, 8.2_
  - [x] 10.3 Assert shebang line `#!/usr/bin/env node` is the first line of both compiled files
    - _Requirements: 1.2, 5.1_
  - [x] 10.4 Run `pnpm pack --dry-run` and assert `src/` is not listed in the output
